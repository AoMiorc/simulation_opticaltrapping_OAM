...
def create_lg61_opposite_phase_field(r, phi, z, w0=2e-6, wavelength=1064e-9, l=6, p=1):
    """
    创建LG61光束强度分布，内环和外环相位相反 (l=6, p=1) 
    Create LG61 beam intensity distribution with opposite phase for inner and outer rings (l=6, p=1)
    ...
    # 内环：l = +6 (逆时针旋转)，外环：l = -6 (顺时针旋转) / Inner ring: l = +6 (counterclockwise), outer ring: l = -6 (clockwise)
...
def test_lg61_opposite_phase_two_particles():
    """测试相位相反的LG61光束中的双粒子动力学 / Test two particle dynamics in opposite phase LG61 beam"""
    print("=== LG61 Opposite Phase Beam Two Particles Test / LG61相位相反光束双粒子测试 ===")
    ...
    # 4. 设置相位相反的LG61光场 / Set up opposite phase LG61 optical field
    ...
    def lg61_opposite_phase_field_function(x, y, z):
        """相位相反的LG61光场函数 / Opposite phase LG61 optical field function"""
        r = np.sqrt(x**2 + y**2)
        phi = np.arctan2(y, x)
        return create_lg61_opposite_phase_field(r, phi, z, w0=optical_trap.w0, 
                                wavelength=optical_trap.wavelength, l=6, p=1)
    
    def lg61_opposite_phase_function(x, y, z):
        """相位相反的LG61相位函数 / Opposite phase LG61 phase function"""
        phi = np.arctan2(y, x)
        r = np.sqrt(x**2 + y**2)
        boundary_radius = 3.5e-6
        ...
        return effective_l * phi  # l=6的相位 / Phase for l=6
    
    # 设置光场 / Set optical field
    optical_trap.set_field(x_range, y_range, z_range, 
                          lg61_opposite_phase_field_function, lg61_opposite_phase_function)
    print("LG61 opposite phase field setup completed / LG61相位相反光场设置完成")
    ...
    df1.to_csv('particle1_trajectory_lg61_opposite_phase.csv', index=False)
    ...
    df2.to_csv('particle2_trajectory_lg61_opposite_phase.csv', index=False)
    
    print("Trajectory data saved / 轨迹数据已保存:")
    print("- Particle 1: particle1_trajectory_lg61_opposite_phase.csv")
    print("- Particle 2: particle2_trajectory_lg61_opposite_phase.csv")
    ...
    for i, traj in enumerate(trajectory):
        ...
        # 分析相位相反效应 / Analyze opposite phase effects
        boundary_radius = 3.5e-6
        if i == 0:  # 内环粒子 / Inner ring particle
            print(f"  Phase region / 相位区域: Inner ring (positive phase) / 内环 (正相位)")
        else:  # 外环粒子 / Outer ring particle
            print(f"  Phase region / 相位区域: Outer ring (negative phase) / 外环 (负相位)")
    
    return trajectory

def visualize_lg61_opposite_phase_two_particles():
    """可视化相位相反的LG61双粒子轨迹 / Visualize opposite phase LG61 two particles trajectory"""
    ...
    field_intensity = lg61_opposite_phase_field_function(X, Y, Z_field)
    
    # 左图：粒子1轨迹 / Left plot: Particle 1 trajectory
    im1 = ax1.contourf(X*1e6, Y*1e6, field_intensity, levels=20, alpha=0.6, cmap='viridis')
    
    # 标记内外环分界线 / Mark boundary between inner and outer rings
    boundary_radius = 3.5
    circle = plt.Circle((0, 0), boundary_radius, fill=False, color='red', linestyle='--', linewidth=2)
    ax1.add_patch(circle)
    
    # 加载并绘制粒子1轨迹 / Load and plot particle 1 trajectory
    try:
        df1 = pd.read_csv('particle1_trajectory_lg61_opposite_phase.csv')
        ax1.plot(df1['X (m)']*1e6, df1['Y (m)']*1e6, 'r-', linewidth=2, label='Particle 1 Trajectory (Inner Ring)')
    ...
    ax1.set_xlabel('X (μm)')
    ax1.set_ylabel('Y (μm)')
    ax1.set_title('LG61 Opposite Phase - Particle 1 (Inner Ring +φ) / LG61相位相反 - 粒子1 (内环 +φ)')
    ax1.legend()
    ...
    # 标记内外环分界线 / Mark boundary between inner and outer rings
    circle2 = plt.Circle((0, 0), boundary_radius, fill=False, color='red', linestyle='--', linewidth=2)
    ax2.add_patch(circle2)
    
    # 加载并绘制粒子2轨迹 / Load and plot particle 2 trajectory
    try:
        df2 = pd.read_csv('particle2_trajectory_lg61_opposite_phase.csv')
        ax2.plot(df2['X (m)']*1e6, df2['Y (m)']*1e6, 'b-', linewidth=2, label='Particle 2 Trajectory (Outer Ring)')
    ...
    ax2.set_xlabel('X (μm)')
    ax2.set_ylabel('Y (μm)')
    ax2.set_title('LG61 Opposite Phase - Particle 2 (Outer Ring -φ) / LG61相位相反 - 粒子2 (外环 -φ)')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    ax2.set_aspect('equal')
    
    plt.tight_layout()
    plt.savefig('lg61_opposite_phase_two_particles_trajectories.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("Visualization saved / 可视化已保存: lg61_opposite_phase_two_particles_trajectories.png")

if __name__ == "__main__":
    # 运行相位相反的LG61双粒子测试 / Run opposite phase LG61 two particles test
    trajectory = test_lg61_opposite_phase_two_particles()
    
    # 可视化结果 / Visualize results
    visualize_lg61_opposite_phase_two_particles()
    
    print("\n=== LG61 Opposite Phase Two Particles Test Completed / LG61相位相反双粒子测试完成 ===")
    print("Results saved / 结果已保存:")
    print("- Particle 1 trajectory / 粒子1轨迹: particle1_trajectory_lg61_opposite_phase.csv")
    print("- Particle 2 trajectory / 粒子2轨迹: particle2_trajectory_lg61_opposite_phase.csv")
    print("- Visualization / 可视化: lg61_opposite_phase_two_particles_trajectories.png")