...
def create_lg10_field(r, phi, z, w0=2e-6, wavelength=1064e-9, l=10, p=0):
    """
    创建LG10光束的强度分布 / Create intensity distribution of LG10 beam
    
    参数 / Parameters:
    r, phi, z: 柱坐标系坐标 / Cylindrical coordinates
    w0: 束腰半径 / Beam waist radius
    wavelength: 波长 / Wavelength
    l: 轨道角动量量子数 (对于LG10，l=10) / Orbital angular momentum quantum number (for LG10, l=10)
    p: 径向量子数 (对于LG10，p=0) / Radial quantum number (for LG10, p=0)
    """
    ...
    # LG10模式的径向部分 (l=10, p=0) / Radial part of LG10 mode (l=10, p=0)
    # 对于p=0，广义拉盖尔多项式L_0^10(ρ²) = 1 / For p=0, generalized Laguerre polynomial L_0^10(ρ²) = 1
    radial_part = rho**10 * np.exp(-rho**2 / 2)
    
    # 角向部分 (l=10) / Angular part (l=10)
    angular_part = np.exp(1j * l * phi)
    
    # 高斯包络 / Gaussian envelope
    gaussian_envelope = (w0 / w_z) * np.exp(-r**2 / w_z**2)
    
    # 完整的LG10场振幅 / Complete LG10 field amplitude
...
def test_lg10_single_particle():
    """测试单粒子在LG10光束中的运动 / Test single particle motion in LG10 beam"""
    
    print("开始LG10单粒子测试... / Starting LG10 single particle test...")
    ...
    # 3. 创建LG10光阱 / Create LG10 optical trap
    optical_trap = OpticalTrap(
        kappa=[1e-7, 1e-7, 1e-7],  # 阱刚度 [N/m] / Trap stiffness [N/m]
        center=np.array([0.0, 0.0, 0.0]),
        wavelength=1064e-9,  # 1064nm激光 / 1064nm laser
        laser_power=1,  # 100mW
        w0=2e-6/3.16,  # 2μm束腰 / 2μm beam waist
        l=10  # LG10的轨道角动量量子数 / Orbital angular momentum quantum number for LG10
    )
    
    # 4. 设置LG10光场 / Setup LG10 optical field
    ...
    def lg10_field_function(x, y, z):
        """LG10光场函数 / LG10 optical field function"""
        r = np.sqrt(x**2 + y**2)
        phi = np.arctan2(y, x)
        return create_lg10_field(r, phi, z, w0=optical_trap.w0, 
                               wavelength=optical_trap.wavelength, l=10, p=0)
    
    def lg10_phase_function(x, y, z):
        """LG10相位函数 / LG10 phase function"""
        phi = np.arctan2(y, x)
        return optical_trap.l * phi  # l=10的相位 / Phase for l=10
    
    optical_trap.set_field(x_range, y_range, z_range, 
                          lg10_field_function, lg10_phase_function)
    print("LG10光场设置完成 / LG10 optical field setup completed")
    ...
    duration = 240  # 240s模拟时间 / 240s simulation time
    ...
    # 7. 保存结果 / Save results
    output_file = "particle_trajectory_lg10_test.csv"
    ...
    print(f"\n=== LG10旋转运动分析 / LG10 Rotational Motion Analysis ===")
    ...
    print(f"LG10绕中心总旋转角度 / LG10 total rotation angle around center: {total_rotation_degrees:.2f}°")
    print(f"LG10绕中心总旋转角度 / LG10 total rotation angle around center: {np.abs(cumulative_rotation):.4f} rad")
    ...
    if actual_simulation_time > 0:
        ...
        print(f"LG10平均绕中心角速度 / LG10 average angular velocity around center: {avg_angular_velocity_around_center:.4f} rad/s")
        print(f"LG10平均绕中心角速度 / LG10 average angular velocity around center: {avg_angular_velocity_around_center * 180 / np.pi:.2f} °/s")
    else:
    ...
    print(f"LG10平均自转角速度 / LG10 average intrinsic angular velocity: {mean_intrinsic_angular_velocity:.4f} rad/s")
    print(f"LG10平均自转角速度 / LG10 average intrinsic angular velocity: {mean_intrinsic_angular_velocity * 180 / np.pi:.2f} °/s")
    ...
    if avg_angular_velocity_around_center > 0:
        ...
        print(f"LG10绕中心旋转频率 / LG10 rotation frequency around center: {rotation_frequency:.6f} Hz")
        if rotation_frequency > 0:
            rotation_period = 1 / rotation_frequency
            print(f"LG10绕中心旋转周期 / LG10 rotation period around center: {rotation_period:.2f} s")
    
    return trajectory, sim_box

# 在test_lg10_single_particle.py的末尾添加 / Add at the end of test_lg10_single_particle.py
if __name__ == "__main__":
    # 运行测试并捕获返回值 / Run test and capture return values
    trajectory, sim_box = test_lg10_single_particle()
    
    # 创建可视化器并加载数据 / Create visualizer and load data
    from simulation.visualizer import TrajectoryVisualizer
    visualizer = TrajectoryVisualizer("particle_trajectory_lg10_test.csv")
    ...
    optical_trap.set_field(x_range, y_range, z_range, lg10_field_function)
