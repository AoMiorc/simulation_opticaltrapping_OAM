...
def test_lp71_with_csv_field_new():
    """Test single particle motion using new CSV optical field data"""
    
    print("Starting LP71 new CSV optical field test...")
    ...
    print("\n=== Motion Analysis ===")
    print(f"Average speed: {motion_analysis['avg_speed']*1e6:.3f} μm/s")
    print(f"Average angular velocity: {motion_analysis['avg_angular_velocity']:.3f} rad/s")
    print(f"Average orbital radius: {motion_analysis['avg_radius']*1e6:.2f} μm")
    print(f"Orbital radius range: {motion_analysis['min_radius']*1e6:.2f} - {motion_analysis['max_radius']*1e6:.2f} μm")
    
    if motion_analysis['orbital_period'] is not None:
        print(f"Estimated orbital period: {motion_analysis['orbital_period']:.3f} s")
        print(f"Estimated orbital frequency: {1/motion_analysis['orbital_period']:.3f} Hz")
    else:
        print("No obvious orbital motion detected")
    
    # Additional statistical information
    print("\n=== Velocity Statistics ===")
    print(f"Velocity standard deviation: {np.std(motion_analysis['velocities'])*1e6:.3f} μm/s")
    print(f"Maximum instantaneous velocity: {np.max(motion_analysis['velocities'])*1e6:.3f} μm/s")
    print(f"Minimum instantaneous velocity: {np.min(motion_analysis['velocities'])*1e6:.3f} μm/s")
    
    print("\n=== Angular Velocity Statistics ===")
    print(f"Angular velocity standard deviation: {np.std(motion_analysis['angular_velocities']):.3f} rad/s")
    print(f"Maximum angular velocity: {np.max(motion_analysis['angular_velocities']):.3f} rad/s")
    print(f"Minimum angular velocity: {np.min(motion_analysis['angular_velocities']):.3f} rad/s")
    ...
    if trajectory is not None:
        ...
        try:
            ...
            if csv_intensity_function is not None and csv_phase_function is not None:
                ...
                # 新增相位背景绘图
                visualizer.plot_2d_trajectory_with_phase('xy', 
                                                        optical_trap=optical_trap, 
                                                        field_alpha=0.6)
        ...
        print("\nLP71 new CSV optical field test completed!")